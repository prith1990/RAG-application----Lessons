...
def main():
    try:
        collection = client.create_collection(
            ...
            metadata={"description": "Collection for storing employee data"},
            configuration={
                "hnsw":{"space" : "cosine"},
    ...
    # Creating the document list for embedding
    employee_documents = []
...
def perform_advanced_search(collection, all_items):
    try:
        ...
        query_text = "Python Developers with Web Development experience"
        ...
        # Example 2: Search for leadershiop roles
        ...
        print(f"Found {len(results['ids'][0])} Engineering employees:")
        ...
        print(f"Found {len(results['ids'])} employees with 10+ years of experience:")
    ...
    print("\n5. Finding employees in California and Los Angeles:")
    results = collection.get(
        where={"location" : {"$in":["San Francisco", "Los Angeles"]}}
    )
    print(f"Found {len(results['ids'])} employees in California and Los Angeles:")
    for i, doc_id in enumerate(results['ids']):
        metadata = results['metadatas'][i]
    ...
    query_text = "Senior Python Developers full-stack"
    ...
    for i, (doc_id, document, distance) in enumerate(zip(results['ids'][0], results['documents'][0], results['distances'][0])):
        metadata = results['metadatas'][0][i]
        print(f"  {i+1}. {metadata['name']}, Experience: {metadata['experience']}, Location: {metadata['location']} ")
