...
def load_food_data(file_path: str) -> List[Dict]:
    """Load food records from a JSON file and normalize them for search indexing.

    Args:
        file_path: Path to the JSON file containing food data.

    Returns:
        A list of normalized food dictionaries.
    """
    try:
        ...
        for i, item in enumerate(food_data):
            # Ensure Food_id is present for all the records
...
def create_simialrity_search_collection(collection_name: str, collection_metadata: dict = None):
    ...
    Args:
        ...
        collection_metadata: Optional metadata to attach to the collection.

    Returns:
        The created Chroma collection object.
    """
...
def populate_similarity_search(collection, food_items: List[Dict]):
    ...
    Args:
        collection: The Chroma collection to populate.
        food_items: A list of food dictionaries containing recipe details.
    """
...
def perform_similarity_search(collection, query: str, n_results: int = 5) -> List[Dict]:
    """Search for food items similar to the given query.
    ...
    Returns:
        A list of formatted result dictionaries with similarity metadata.
    """
    try:
...
def perform_filtered_similarity_search(collection, query: str, cuisine_filter: str = None, 
    ...
    """Perform similarity search with optional cuisine and calorie filters.

    Args:
        collection: The Chroma collection to query.
        query: The search text entered by the user.
        cuisine_filter: Optional cuisine type to restrict results to.
        max_calories: Optional maximum calorie threshold for results.
        n_results: The maximum number of results to return.

    Returns:
        A list of formatted result dictionaries matching the filters.
    """
    where_clause = None
